...
[← User Guide](../README.md) · [Workspaces](../README.md#the-five-workspaces) · [Next: Modify Frame →](modify-frame.md)
...
- **Right** — vertical [timeline](../timeline-and-tracks.md): source track at the top, render tracks stacked below.

## Source video selector

At the top of the Settings panel — a dropdown of all videos in the current shot. By default the original source is selected. **Any render in the shot can be used as a source for further renders**, which is the basis of the [multi-pass workflow](../workflows-recipes.md#recipe-multi-pass-pipeline).
...
The single most important setting — the chosen model determines what the rest of the panel exposes. See the [Model catalog](../../models/README.md) and the [decision tree](../../models/README.md#decision-tree).
...
- **Prompt** — describes the desired result (Mago) or instructs the model (closed-source). See [Prompting](../prompting-guide.md).
...
- **ControlNets** (Mago Transform only) — depth, pose, softedge (beta), depth+pose, normal map (beta). See [Mago Transform → ControlNets](../../models/mago-video-models.md#controlnets-for-mago-transform).
...
| **Image sequence export** | PNG or EXR 16-bit. EXR is for [VFX pipelines](../export-and-compositing.md#exr-for-vfx-pipelines) (Mago Transform & Style Transfer). |
...
- **Generate (with frame range)** — renders only the [range](../timeline-and-tracks.md#range-selector) selected on the timeline. Shows credit cost and frame numbers.
