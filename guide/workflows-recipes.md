...
[← Prompting guide](/guide/prompting-guide) · [User Guide](/guide/overview) · [Next: Troubleshooting →](/guide/troubleshooting)
...
2. Open [Modify Frame](/guide/workspaces/modify-frame). Select a representative frame (usually one where the actor's face is visible).
...
1. Open the [Mask](/guide/workspaces/mask) workspace.
...
11. If unmasked regions show slight changes (brightness shift), plan to [composite externally](/guide/export-and-compositing#mask-export) with the downloaded mask for pixel-perfect preservation.
